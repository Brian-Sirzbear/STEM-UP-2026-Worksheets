...
class LibraryViewer:
    ...
    def create_widgets(self):
        ...
        self.examples_box = widgets.GridBox(
            children=[],
        ...
        self.btn_save_examples = widgets.Button(
        ...
        self.btn_cancel_examples = widgets.Button(
            description='Cancel',
            button_style='warning'
        )
        
        self.btn_save_examples.on_click(
            self.save_examples
        )

        self.btn_add_example.on_click(self.add_example_row)

        self.btn_cancel_examples.on_click(
            self.cancel_examples
        )
        
        self.examples_editor = widgets.VBox([
            self.btn_add_example,
            self.examples_box,
            widgets.HBox([
                self.btn_save_examples,
                self.btn_cancel_examples
            ])
        ])
        
        self.examples_editor.layout.display = "none"
    ...
    def display_entry(self, entry, index):
        ...
        btn_edit_inputs = widgets.Button(
            ...
            button_style='info',
            icon='sliders',
            layout=widgets.Layout(width='111px')
        )

        btn_edit_examples = widgets.Button(
            description='Edit Examples',
        ...
        btn_edit_examples.on_click(
            lambda b, i=index: self.edit_examples(i)
        )
    ...
    def create_input_row(self, name="", rep="", typ="---", status="---", assumed=""):
        
        name_w = widgets.Text(value=name, description='Input:', layout=widgets.Layout(width='180px'))
        rep_w = widgets.Text(value=rep, description='Represents:', layout=widgets.Layout(width='500px'))
    
        typ_w = widgets.Dropdown(
            options=['---','integer','float','integer/float','string','list','array','list/array','table','boolean','anything'],
            value=typ,
            description='Type:',
            layout=widgets.Layout(width='220px')
        )
        ...
        assumed_w = widgets.Text(value=assumed, description='Assumed:', layout=widgets.Layout(width='220px'))
    
        row = widgets.HBox([name_w, rep_w, typ_w, status_w, assumed_w])
    
        self.input_rows.append((name_w, rep_w, typ_w, status_w, assumed_w))
    
        return row

    def add_input_row(self, _):
        new_row = self.create_input_row()
        self.inputs_editor_box.children = list(self.inputs_editor_box.children) + [new_row]

    def style_example_row(self, row):
    ...
    def edit_examples(self, index):
    
        self.example_rows = []
        self.examples_box.children = ()
        self.w_examples_edit_index = index
        ...
        if not rows:
            rows.append(self.create_example_row())
    
        self.examples_box.children = rows
        self.examples_editor.layout.display = "block"
        
    def create_example_row(self, code="", output="", source=""):
        ...
        entry = (code_w, output_w, source_w, card)
    
        self.example_rows.append(entry)
    
        def delete_row(_):
            # remove from tracking list
            if entry in self.example_rows:
                self.example_rows.remove(entry)
    
            # rebuild grid safely
            self.examples_box.children = tuple(
                e[3] for e in self.example_rows
            )
    ...
    def add_example_row(self, _):
        new_card = self.create_example_row()
    
        # ONLY rebuild from source of truth
        self.examples_box.children = tuple(
            e[3] for e in self.example_rows
        )
    ...
    def save_edit(self, _):
        ...
        self.data[index] = {
            ...
            "Examples": self.data[index].get("Examples", []),
    ...
    def save_examples(self, _):

        index = self.w_examples_edit_index
    
        new_examples = []
    
        for code, output, source, _ in self.example_rows:
